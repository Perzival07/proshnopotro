...
import { isDriveFileUrl } from "@/lib/test-resource";
...
export interface NoteCardData {
  ...
  /** Photographed pages. PDFs are never uploaded; they arrive as the link. */
  fileCount: number;
...
export function NoteCard({ note }: { note: NoteCardData }) {
  const linksPdf = Boolean(note.linkUrl && isDriveFileUrl(note.linkUrl));
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-[11px]">
        {note.fileCount > 0 && (
          <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-border bg-brand-page px-2.5 py-1 font-medium text-brand-ink/75">
            ...
            {note.fileCount} page{note.fileCount === 1 ? "" : "s"}
          </span>
        )}
        {note.linkUrl && (
          <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-border bg-brand-page px-2.5 py-1 font-medium text-brand-ink/75">
            {linksPdf ? <FileText className="h-3 w-3" /> : <Link2 className="h-3 w-3" />}
            {linksPdf ? "PDF" : "Link"}
          </span>
        )}
